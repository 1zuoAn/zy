...
from app.schemas.request.schedule_request import ScheduleStartRequest, ScheduleTaskRequest
...
from app.service.chains.workflow.deepresearch.abroad.abroad_deepresearch_graph import (
    AbroadDeepresearchGraph,
)
from app.service.chains.workflow.deepresearch.douyi.douyi_deepresearch_graph import (
    DouyiDeepresearchGraph,
)
from app.service.chains.workflow.deepresearch.ins import InsDeepresearchGraph
from app.service.chains.workflow.deepresearch.zhiyi.zhiyi_deepresearch_graph import (
...
from app.service.chains.workflow.schedule.schedule_graph import (
    ScheduleStartGraph,
    ScheduleTaskGraph,
)
...
@router.post("/image-search", response_model=CommonResponse[WorkflowResponse])
def run_image_search_workflow(request: WorkflowRequest):
    """图搜工作流入口"""
    state = ImageSearchGraph().run(request)
    return CommonResponse[WorkflowResponse](result=state.get("workflow_response"))

# ==================== 店铺与定时任务工作流 ====================

@router.post("/shop", response_model=CommonResponse[WorkflowResponse])
def run_shop_workflow(request: WorkflowRequest):
    """店铺工作流入口"""
    state = ShopGraph().run(request)
    return CommonResponse[WorkflowResponse](result=state.get("workflow_response"))


@router.post("/schedule/start", response_model=CommonResponse[WorkflowResponse])
def run_schedule_start_workflow(request: ScheduleStartRequest):
    """定时任务开始通知入口（对齐 n8n scheduled start）。"""
    state = ScheduleStartGraph().run(request)
    return CommonResponse[WorkflowResponse](result=state.get("workflow_response"))


@router.post("/schedule/task", response_model=CommonResponse[WorkflowResponse])
def run_schedule_task_workflow(request: ScheduleTaskRequest):
    """定时任务消息下发入口（对齐 n8n scheduled tasks）。"""
    state = ScheduleTaskGraph().run(request)
    return CommonResponse[WorkflowResponse](result=state.get("workflow_response"))
...
@router.post("/ins-deepresearch", response_model=CommonResponse[WorkflowResponse])
def _run_ins_deepresearch_workflow(request: WorkflowRequest):
    state = InsDeepresearchGraph().run(request)
    return CommonResponse[WorkflowResponse](result=state.get("workflow_response"))
