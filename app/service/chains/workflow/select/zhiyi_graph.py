...
class ZhiyiGraph(BaseWorkflowGraph):
    ...
    def _build_llm_parameters_payload(
        ...
        *,
        ...
        return [
            {"propertyList": property_list or []},
            {"output": param_output},
            {"output": sort_output},
            {"output": {"values": user_style_values or ""}},
            {"output": {"user_tags": user_filter_tags or []}},
            {"output": brand_output},
        ]

    def _insert_llm_parameters(self, req: WorkflowRequest, payload: list[dict[str, Any]]) -> None:
        ...
        def _insert():
            with pg_session(DBAlias.DB_ABROAD_AI.value) as session:
                params = {
                    ...
                    "user_id": req.user_id,
                    "team_id": req.team_id,
                    "llm_parameters": json.dumps(payload, ensure_ascii=False),
                }
                result = session.execute(
                    text(
                        """
    ...
    def _llm_param_parse_node(self, state: ZhiyiWorkflowState) -> dict[str, Any]:
        ...
        messages: list[BaseMessage] = coze_loop_client_provider.get_langchain_messages(
            prompt_key=CozePromptHubKey.ZHIYI_MAIN_PARAM_PARSE_PROMPT.value,
        ...
        structured_llm = llm.with_structured_output(ZhiyiParseParam).with_retry(
            stop_after_attempt=2
        )
        param_result: ZhiyiParseParam = structured_llm.with_config(run_name="主参数解析").invoke(
            messages
        )
