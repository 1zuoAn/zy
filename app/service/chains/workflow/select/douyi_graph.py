...
节点清单（12 个）：
- 预处理: init_state, pre_think, query_selections, llm_parse, parallel_parse
- 主 API: api_strict
- 兜底 API: api_fallback
- 汇聚: merge_result
...
改进点：
- 将 llm_sort_parse 并入 parallel_parse 线程池
- 拆解隐式兜底为显式 api_strict -> check -> api_fallback -> merge 结构
- 添加独立 post_process 节点
"""
...
from app.schemas.entities.workflow.llm_output import (
    DouyiParseParam,
    DouyiPropertyListParseResult,
    DouyiSortTypeParseResult,
    DouyiUserTagResult, DouyiPropertyLlmCleanResult,
)
...
from app.service.rpc.zhiyi_api import DouyiGoodsEntity, DouyiSearchRequest, get_zhiyi_api_client
...
class DouyiGraph(BaseWorkflowGraph):
    ...
    def _llm_param_parse_node(self, state: DouyiWorkflowState) -> dict[str, Any]:
        ...
        messages: list[BaseMessage] = coze_loop_client_provider.get_langchain_messages(
            prompt_key=CozePromptHubKey.DOUYI_MAIN_PARAM_PARSE_PROMPT.value,
        ...
        structured_llm = llm.with_structured_output(DouyiParseParam).with_retry(
            stop_after_attempt=2
        )

        param_result = structured_llm.with_config(run_name="参数解析节点").invoke(messages)
        
        # 如果有店铺引用，清除 LLM 可能误提取的品牌参数（避免店铺名被当成品牌）
        if shop_id and param_result.brand:
            logger.debug(f"[抖衣] 检测到店铺引用，清除LLM提取的品牌参数: {param_result.brand}")
            param_result.brand = None
        
        # 如果 LLM 没有解析出品牌，但我们检测到了品牌引用，则强制设置
        # 注意：只在有品牌引用时设置，避免把店铺名当成品牌
        ...
        param_result = self._normalize_param_result(param_result)
        logger.debug(
            f"[抖衣] Step 4: llm_parse - 解析结果: category={param_result.category_id_list}, price={param_result.min_price}-{param_result.max_price}, brand={param_result.brand}"
        )
    ...
    def _parallel_parse_node(self, state: DouyiWorkflowState) -> dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            if param_result.properties and param_result.properties.strip():
                futures["properties"] = _submit(
                    self._parse_properties, format_query, param_result.properties
                )

            # 用户画像 + 标签
            if param_result.user_data == 1:
                futures["profile"] = _submit(self._fetch_user_profile_tags, req.user_id)

            # 排序解析（并入线程池）
            futures["sort"] = _submit(
    ...
    def _parse_properties(self, origin_text: str, properties_text: str) -> list[list[str]]:
        """解析属性（对齐 n8n 封装多属性）"""
        try:
            ...
            response = kb_client.simple_chat(query=properties_text, service_resource_id=VolcKnowledgeServiceId.DOUYI_PROPERTIES_VECTOR.value)
            ...
            messages = coze_loop_client_provider.get_langchain_messages(
                ...
                variables={
                    "query_tag_list": json.dumps(match_tags, ensure_ascii=False),
    ...
    def _fetch_user_filters(
        self, user_id: int, root_category_id: int, category_id_list: str
    ) -> list[Any]:
        ...
        """
        try:
            # 解析 category_id_list (逗号分隔的字符串 -> 列表)
            category_ids = [
                int(cid.strip()) for cid in category_id_list.split(",") if cid.strip()
            ]
            if not category_ids:
                return []
    ...
    def _api_strict_node(self, state: DouyiWorkflowState) -> dict[str, Any]:
        ...
        property_list = state.get("property_list", [])
        ...
        # 设置店铺ID（如果有店铺引用）
        if shop_id:
            api_request.shop_id = shop_id
            logger.debug(f"[抖衣] 设置店铺ID: {shop_id}")
        ...
        try:
            page_result: PageResult[DouyiGoodsEntity] = zhiyi_client.common_search_douyi(
    ...
    def _has_result_node(self, state: DouyiWorkflowState) -> dict[str, Any]:
        ...
        req = state["request"]
        api_request: DouyiSearchRequest = state["api_request"]
        param_result: DouyiParseParam = state["param_result"]
