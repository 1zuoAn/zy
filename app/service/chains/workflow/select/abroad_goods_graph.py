...
from app.schemas.entities.workflow.llm_output import (
    ...
    AbroadGoodsSortResult, AbroadGoodsBrandResult,
)
...
class AbroadGoodsGraph(BaseWorkflowGraph):
    ...
    def _query_common_selection_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        if platform_type_override is None:
            ...
            if platform_refs:
                ...
                if platform_name:
                    ...
                    if "amazon" in name_lower or "亚马逊" in platform_name:
                        platform_zone_type = "amazon"
                    elif "temu" in name_lower:
                        platform_zone_type = "temu"
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            # 并行执行：筛选项 API + 类目向量检索
            selection_future = executor.submit(get_abroad_api().get_common_selections)
            category_vector_future = executor.submit(
    ...
    def _extract_user_profile_tags(self, user_profile: dict[str, Any]) -> list[str]:
        """从用户画像提取标签"""
        style = str(user_profile.get("style") or "").strip()
        if not style:
            return []
    ...
    ) -> list[UserTagFilterItem]:
        """获取用户筛选项"""
        if not root_category_id or not category_id_list:
            return []
        try:
            with pg_session(DBAlias.DB_ABROAD_AI.value) as session:
                row = (
                    session.execute(
                        ...
                        and category_id = any(:category_id_list)
            ...
            if not row or not row.get("filters"):
                return []
            raw = row["filters"]
            if not row:
                return []
            ...
            return [UserTagFilterItem(**item) for item in raw if isinstance(item, dict)]
        except Exception as e:
            logger.warning(f"[海外探款商品] filters 获取失败: {e}")
            return []
    ...
    def _llm_param_parse_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        prompt_param = {
            ...
            "common_selections": json.dumps(
                selection_dict["common_selections"], ensure_ascii=False
            ),
        ...
        messages: list[BaseMessage] = coze_loop_client_provider.get_langchain_messages(
            prompt_key=CozePromptHubKey.ABROAD_GOODS_MAIN_PARAM_PARSE_PROMPT.value,
        ...
        structured_llm = llm.with_structured_output(AbroadGoodsParseParam).with_retry(
        ...
        if param_result:
            ...
            if normalized_categories != getattr(param_result, "category_id_list", []):
                logger.debug(
                    f"[海外探款] 类目路径校验完成: {normalized_categories}"
                )
    ...
    def _parallel_parse_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        root_category_id, category_ids = self._derive_root_and_category_ids(
            getattr(param_result, "category_id_list", []) or []
        )

        futures = {}
        with ThreadPoolExecutor(max_workers=3) as executor:
            # 排序解析
            futures["sort"] = executor.submit(self._parse_sort, req.user_query, param_result)

            # 用户画像标签 (对应 n8n 的 "封装用户画像标签"，仅 user_data=1 时启用)
            if getattr(param_result, "user_data", 0) == 1:
                futures["user_profile"] = executor.submit(self._fetch_user_style, req.user_id)
                if root_category_id and category_ids:
                    futures["user_filters"] = executor.submit(
        ...
        for key, future in futures.items():
            try:
                ...
                if key == "sort":
                    ...
                    # n8n 中该结果不进入 API 参数，暂时跳过处理
                    # 保留用于必要时扩展
                    pass
                elif key == "user_filters":
    ...
    def _site_detail_search_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        try:
            ...
            request_path = result.get("final_request_path") or result.get(
                "secondary_request_path" if used_fallback else "primary_request_path"
            )
            request_params = result.get("final_params") or result.get(
    ...
    def _amazon_search_goods_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        request_path = result.get(
            "fallback_request_path" if used_fallback else "request_path"
        ) or result.get("final_request_path") or "amazon/goods/list"
        request_params = result.get(
            "fallback_params" if used_fallback else "api_params"
        ) or result.get("final_params")
        request_body = (
            json.dumps(request_params, ensure_ascii=False) if request_params is not None else None
        )

        # 构建兼容的 api_request（用于后续节点）
        api_request = AbroadGoodsSearchRequest.from_parse_param(param_result, sort_type=sort_type)
    ...
    def _temu_search_goods_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        try:
            ...
            request_path = result.get(
                "fallback_request_path" if used_fallback else "request_path"
            ) or result.get("final_request_path") or "temu/goods/list"
            request_params = result.get(
                "fallback_params" if used_fallback else "api_params"
            ) or result.get("final_params")
            request_body = (
    ...
    def _merge_search_result_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        返回:
            ...
            browsed_count: 总浏览数量
            api_request: 最终请求体
            final_request_path: 最终请求路径
            final_request_body: 最终请求体 JSON
        ...
        final_request_path = fallback_request_path if fallback_used_here else request_path
        final_request_body = fallback_request_body if fallback_used_here else request_body
        if not final_request_body and final_request is not None:
            if hasattr(final_request, "model_dump_json"):
                final_request_body = final_request.model_dump_json(by_alias=True, exclude_none=True)
            elif isinstance(final_request, dict):
                final_request_body = json.dumps(final_request, ensure_ascii=False)
    ...
    def _has_result_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        """
        req = state["request"]
        api_request: AbroadGoodsSearchRequest = state.get("api_request")
        param_result: AbroadGoodsParseParam = state["param_result"]
        merged_api_resp = state.get("merged_api_resp")
        ...
        request_path = (
            state.get("final_request_path")
            or state.get("request_path")
            or "goods-center/goods-zone-list"
        )
        request_body = state.get("final_request_body") or state.get("request_body")
    ...
    def _package_result_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        request_path = state.get("final_request_path") or state.get("request_path", "")
        request_body = state.get("final_request_body") or state.get("request_body", "")
