...
class VolcengineKBAPI:
    ...
    # 解析知识库结果的辅助方法
    # =====================
    def parse_structure_chat_response(self, response: KBServiceChatResponse) -> list[dict[str, str]]:
        """
        解析结构化知识库（excel/csv）的response结果，转化为字典返回
        """
        ...
        for item in response.data.result_list:
            ...
            for chunk in item.table_chunk_fields:
                ...
                single_row[field_name] = field_value
            content_list.append(single_row)
        return content_list
