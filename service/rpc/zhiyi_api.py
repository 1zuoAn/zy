...
from app.schemas.entities.workflow.llm.douyi_output import DouyiParseParam
from app.schemas.entities.workflow.llm.zhiyi_output import ZhiyiParseParam
...
class DouyiSearchRequest(BaseModel):
    ...
    def from_parse_param(cls, param: DouyiParseParam) -> "DouyiSearchRequest":
        ...
        raw_category_ids = param.category_id_list or []
        if isinstance(raw_category_ids, str):
            for item in raw_category_ids.split(","):
                text = item.strip()
                if text.isdigit():
                    category_id_list.append(int(text))
        else:
            for item in raw_category_ids:
                if isinstance(item, int):
                    category_id_list.append(item)
                    continue
                text = str(item).strip()
                if text.isdigit():
                    category_id_list.append(int(text))
        ...
        return cls(
            ...
            categoryIdList=category_id_list or None,
...
class DouyiGoodsListRequest(BaseModel):
    """抖衣商品库列表请求参数（goods-list）"""

    class Config:
        populate_by_name = True

    # 品类（goods-list 口径为一维数组）
    ...
    # 收录时间范围
    sort_start_time: Optional[str] = Field(default=None, alias="sortStartTime")
    sort_end_time: Optional[str] = Field(default=None, alias="sortEndTime")

    # 排序
    sort_field: Optional[int] = Field(default=None, alias="sortField")
    ...
    # 其他筛选
    ship_address: Optional[str] = Field(default=None, alias="shipAddress")
    hot_flag: int = Field(default=0, alias="hotFlag")
    is_export: int = Field(default=1, alias="isExport")
    is_hot_style_rank: Optional[int] = Field(default=None, alias="isHotStyleRank")
    is_live_item: Optional[int] = Field(default=None, alias="isLiveItem")
    is_video_item: Optional[int] = Field(default=None, alias="isVideoItem")
    is_card_item: Optional[int] = Field(default=None, alias="isCardItem")
    is_today_item: Optional[int] = Field(default=None, alias="isTodayItem")
    is_today_record: Optional[int] = Field(default=None, alias="isTodayRecord")
    is_filter_collected: Optional[int] = Field(default=None, alias="isFilterCollected")
...
class ZhiyiAPI:
    ...
    def get_category_property_options(
        ...
        *,
        ...
        注意：
        - 知衣后端不同环境/版本可能存在字段差异，因此这里返回原始结构 list[dict]，
          由调用方做兼容解析与筛选。
        - 若接口不可用或返回非预期结构，抛出异常由调用方决定是否降级。
        """
        payload: dict[str, Any] = {
            "rootCategoryIdList": [root_category_id] if root_category_id else None,
            "categoryIdList": category_id_list or None,
        }
        data = self._request(
            method="POST",
            path="/v2-0-x/item/property/list",
            user_id=user_id,
            team_id=team_id,
    ...
    def goods_list_douyi(
        self,
        user_id: int,
        team_id: int,
        params: DouyiGoodsListRequest,
    ) -> PageResult[DouyiGoodsEntity]:
        """抖音商品库列表（goods-list）"""
        data = self._request(
            method="POST",
            path="/live-bus/douyin/window-goods/goods-list",
            user_id=user_id,
            team_id=team_id,
            json=params.model_dump(by_alias=True, exclude_none=True),
        )
