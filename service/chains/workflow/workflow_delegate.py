...
def _register_all_workflows(delegate: WorkflowDelegate) -> None:
    ...
    delegate.register(
        WorkflowType.SELECT,
        lambda: _create_select_agent_workflow()
    )

    # === 媒体路由工作流 ===
    delegate.register(
        WorkflowType.MEDIA,
        lambda: _create_media_agent_workflow()
    )
    ...
    # === 图片设计路由工作流 ===
    delegate.register(
        WorkflowType.IMAGE_DESIGN,
        lambda: _create_image_design_workflow()
    )

    # === 图搜工作流 ===
    delegate.register(
        WorkflowType.IMAGE_SEARCH,
        lambda: _create_image_search_workflow()
    )

    # === 店铺排行工作流 ===
    delegate.register(
        WorkflowType.SHOP,
        lambda: _create_shop_workflow()
    )

    # === 定时任务工作流 ===
    delegate.register(
        WorkflowType.SCHEDULE,
        lambda: _create_schedule_workflow()
    )
...
def _create_select_agent_workflow():
    """延迟导入并创建 SelectAgentGraph 实例"""
    from app.service.chains.workflow.select.select_agent_graph import SelectAgentGraph
    return SelectAgentGraph()


def _create_media_agent_workflow():
    """延迟导入并创建 MediaAgentGraph 实例"""
    from app.service.chains.workflow.select.media_agent_graph import MediaAgentGraph
    return MediaAgentGraph()
...
def _create_image_design_workflow():
    """延迟导入并创建 ImageDesignGraph 实例"""
    from app.service.chains.workflow.image.image_design_graph import ImageDesignGraph
    return ImageDesignGraph()


def _create_image_search_workflow():
    """延迟导入并创建 ImageSearchGraph 实例"""
    from app.service.chains.workflow.image_search.image_search_graph import ImageSearchGraph
    return ImageSearchGraph()

def _create_shop_workflow():
    """延迟导入并创建 ShopGraph 实例"""
    from app.service.chains.workflow.shop.shop_graph import ShopGraph
    return ShopGraph()


def _create_schedule_workflow():
    """延迟导入并创建 ScheduleGraph 实例"""
    from app.service.chains.workflow.schedule.schedule_graph import ScheduleGraph
