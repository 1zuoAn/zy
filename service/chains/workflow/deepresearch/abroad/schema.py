...
class AbroadMainParseParam(BaseModel):
    ...
    platform: str | None = Field(
        default=None,
        description="站点平台：SHEIN/Temu/独立站/亚马逊。海外探款这个词除外"
    )
    table_type: int = Field(
        default=1,
        description="图表类型：1-品类分析（默认，查询维度多）、2-非品类分析（属性/颜色/面料/价格带等，查询维度少）"
    )
...
class AbroadDimensionInfoItem(BaseModel):
    ...
    sale_volume: int = Field(default=0, alias="saleVolume", description="销量")
    sale_amount: int = Field(default=0, alias="saleAmount", description="销售额")
    sale_volume_ratio: float = Field(default=0.0, alias="saleVolumeRatio", description="销量占比")
    sale_amount_ratio: float = Field(default=0.0, alias="saleAmountRatio", description="销售额占比")
...
class AbroadAggregatedData(BaseModel):
    ...
    # 趋势数据
    trend_data: AbroadTrendCleanResponse | None = Field(
        default=None, alias="trendData", description="趋势数据"
    )

    # 品类数据
    category_data: AbroadDimensionInfoCleanResponse | None = Field(
        default=None, alias="categoryData", description="品类分布数据"
    )

    # 颜色数据
    color_data: AbroadColorCleanResponse | None = Field(
        default=None, alias="colorData", description="颜色分布数据"
    )

    # 价格带数据
    price_data: AbroadPriceCleanResponse | None = Field(
        default=None, alias="priceData", description="价格带分布数据"
    )

    # 属性数据（面料、袖长等）
    property_data: AbroadPropertyCleanResponse | None = Field(
        default=None, alias="propertyData", description="属性分布数据"
    )

    # Top商品数据
    top_goods: AbroadTopGoodsCleanResponse | None = Field(
        default=None, alias="topGoods", description="Top10商品数据"
    )
...
class AbroadDimensionInfoSummary(BaseModel):
    ...
    sale_volume: int = Field(default=0, alias="saleVolume", description="总销量")
    sale_amount: int = Field(default=0, alias="saleAmount", description="总销售额")
    sale_volume_mom_ratio: float | None = Field(default=None, alias="saleVolumeMomRatio", description="销量环比")
    sale_amount_mom_ratio: float | None = Field(default=None, alias="saleAmountMomRatio", description="销售额环比")
    sale_volume_yoy_ratio: float | None = Field(default=None, alias="saleVolumeYoyRatio", description="销量同比")
    sale_amount_yoy_ratio: float | None = Field(default=None, alias="saleAmountYoyRatio", description="销售额同比")
    goods_num: int = Field(default=0, alias="goodsNum", description="商品数")
    shop_num: int = Field(default=0, alias="shopNum", description="店铺数")
...
class AbroadPropertyRawResponse(BaseModel):
    ...
    error_code: str | None = Field(default=None, alias="errorCode", description="错误码")
    error_desc: str | None = Field(default=None, alias="errorDesc", description="错误描述")
    result: list[AbroadPropertyRawItem] = Field(default_factory=list, description="属性数据列表")
