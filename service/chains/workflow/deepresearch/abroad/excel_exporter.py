...
class AbroadExcelExporter:
    ...
    def __init__(self, aggregated_data: AbroadAggregatedData, param: AbroadMainParseParam):
    ...
    def build_excel_data_list(self) -> List[ExcelData]:
        ...
        # Top10商品
        if self.data.top_goods and self.data.top_goods.result:
    ...
    def _build_price_sheet(self) -> SheetData:
        ...
        for row_idx, item in enumerate(price_items, start=1):
            ...
            if right_usd is not None:
                price_range = f"${left_usd:.0f}-${right_usd:.0f}"
            else:
                price_range = f"${left_usd:.0f}以上"
    ...
    def _build_top_goods_sheet(self) -> SheetData:
        ...
        columns = [
            ...
            ColumnInfo(col_name="销量(30天)", col_id=5, col_type=EXPORT_COLUMN_TYPE_STRING),
            ColumnInfo(col_name="销售额(USD)", col_id=6, col_type=EXPORT_COLUMN_TYPE_STRING),
            ColumnInfo(col_name="价格(USD)", col_id=7, col_type=EXPORT_COLUMN_TYPE_STRING),
        ]
        ...
        for row_idx, item in enumerate(top_goods, start=1):
            ...
            cells = [
                ...
                CellData(col_id=4, cell_value=item.category_detail or item.category_name or ""),
                CellData(col_id=5, cell_value=str(item.sales_volume)),
                CellData(col_id=6, cell_value=str(item.sales_amount)),
