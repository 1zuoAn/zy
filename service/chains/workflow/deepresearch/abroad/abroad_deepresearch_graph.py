...
class AbroadDeepresearchGraph(BaseWorkflowGraph):
    ...
    def _build_graph(self) -> CompiledStateGraph:
        ...
        # 因为有些分支是固定的，有些是动态的，所以要在路由节点内部就确定分支
        graph.add_conditional_edges(
            NodeKey.PROPERTY_DIMENSION_ANALYZE,
            self._route_by_dimension_type
        )

        # 非品类分析分支：聚合 (1. 条件展开的分支不能用list作为start key 2. 这里的开始节点由_route_by_dimension_type内部直接定义)
        graph.add_edge(NodeKey.PROPERTY_OVERVIEW_QUERY, NodeKey.PROPERTY_AGGREGATE)
        graph.add_edge(NodeKey.PROPERTY_CATEGORY_QUERY, NodeKey.PROPERTY_AGGREGATE)
        graph.add_edge(NodeKey.PROPERTY_TOP_GOODS_QUERY, NodeKey.PROPERTY_AGGREGATE)
    ...
    def _site_search_node(self, state: AbroadDeepresearchState) -> dict:
        ...
        logger.info(f"[海外探款] 站点检索 - 用户查询: {req.user_query}, 提取站点名: {parse_param.extract_platform_name}")

        # 向量搜索站点
        search_content = parse_param.extract_platform_name if parse_param.extract_platform_name else req.user_query
        kb_client = get_volcengine_kb_api()
        resp = kb_client.simple_chat(
    ...
    def _route_by_dimension_type(self, state: AbroadDeepresearchState) -> list[str]:
        ...
        dimension_node = ""
        if dimension_type == 1:
            dimension_node = NodeKey.PROPERTY_DIM_COLOR_QUERY
        elif dimension_type == 2:
            dimension_node = NodeKey.PROPERTY_DIM_FABRIC_QUERY
        elif dimension_type == 3:
            dimension_node = NodeKey.PROPERTY_DIM_PRICE_QUERY
        else:
            dimension_node = NodeKey.PROPERTY_DIM_OTHER_QUERY

        return base_nodes + [dimension_node]
    ...
    def _get_api_params(self, state: AbroadDeepresearchState) -> dict:
        ...
        return {
            ...
            "trend_date_granularity": parse_param.trend_date_type if parse_param else "WEEK",
        }
    ...
    def _query_overview_data(self, state: AbroadDeepresearchState) -> dict:
        ...
        request = AbroadTrendSummaryRequest(
            ...
            dateGranularity=params["trend_date_granularity"],
        )
    ...
    def _query_price_data(self, state: AbroadDeepresearchState) -> dict:
        ...
        api = get_abroad_api()
        # 获取页面的默认价格带
        agg_price_resp = api.get_agg_price_range(
            user_id=params["user_id"],
            team_id=params["team_id"],
            request=AbroadAggPriceRangeRequest(platformTypeList=params["platform_type_list"], categoryType="normal", categoryIdList=params["category_id_list"]),
        )
        ...
        # 添加价格带单独参数
        if agg_price_resp and agg_price_resp.result:
            request.price_trend_min = agg_price_resp.result.price_min
            request.price_trend_max = agg_price_resp.result.price_max
            request.price_trend_num = agg_price_resp.result.band_num
    ...
    def _query_top_goods_data(self, state: AbroadDeepresearchState) -> dict:
        ...
        # 构建 Request 对象
        request = AbroadGoodsSearchRequest(
            platformTypeList=params["platform_type_list"],
            categoryIdList=params["category_id_list"],
            startDate=params["start_date"],
            endDate=params["end_date"],
        ...
        # 调用 API
        api = get_abroad_api()
        goods_raw_resp = api.search_goods(
            user_id=params["user_id"],
            team_id=params["team_id"],
            params=request,
        )
        ...
        # 数据清洗
        clean_data = self._clean_top_goods_data(goods_raw_resp)
        logger.info(f"[海外探款] Top商品查询完成 - success={clean_data.success}")

        # 根据 table_type 返回不同的状态字段
        table_type = parse_param.table_type if parse_param else 1
        if table_type == 1:
            return {"category_top_goods": clean_data, "category_top_goods_raw": goods_raw_resp}
        else:
            return {"property_top_goods": clean_data, "property_top_goods_raw": goods_raw_resp}
    ...
    def _query_dim_fabric_data(self, state: AbroadDeepresearchState) -> dict:
        ...
        # 数据清洗
        clean_data = self._clean_fabric_to_property_data(raw_response)
    ...
    def _clean_color_data(
        ...
        items = [
            AbroadColorSlimItem(
                ...
                mom=item.sale_volume_mom_ratio,
    ...
    def _clean_top_goods_data(
        self, page_result: PageResult[AbroadGoodsEntity]
    ) -> AbroadTopGoodsCleanResponse:
        """清洗 search_goods 接口响应（PageResult[AbroadGoodsEntity]）"""
        if not page_result or not page_result.result_list:
            return AbroadTopGoodsCleanResponse(success=False, result=None)

        items = []
        for item in page_result.result_list[:10]:  # 只取前10
            metric = item.metric_info
            items.append(AbroadTopGoodsSlimItem(
                item_id=item.goods_id,
                title=item.goods_name,
                pic_url=item.goods_img,
                category_name=item.category_name,
                category_detail=None,  # 新接口无此字段
                sales_volume=metric.sale_volume_30day if metric and metric.sale_volume_30day else 0,
                sales_amount=metric.sale_amount_30day if metric and metric.sale_amount_30day else 0,
                min_price=str(metric.min_price) if metric and metric.min_price else None,
                max_s_price=int(item.sprice) if item.sprice else None,
            ))
        ...
        return AbroadTopGoodsCleanResponse(
            ...
            result=AbroadTopGoodsSlimResult(
                start=0,
                page_size=page_result.page_size or 10,
    ...
    def _clean_fabric_to_property_data(
        self, raw_response: AbroadDimensionInfoRawResponse
    ) -> AbroadPropertyCleanResponse:
        """将面料数据转换为属性数据格式（用于非品类分支）"""
        if not raw_response.success or not raw_response.result:
            return AbroadPropertyCleanResponse(success=False, result=[])

        items = [
            AbroadPropertySlimItem(
                propertyValue=item.dimension_info or "",
                salesVolume=item.sale_volume,
                salesAmount=item.sale_amount,
                rate=item.sale_volume_ratio,
                trends=[],
            )
            for item in raw_response.result.data_list
    ...
    def _aggregate_category_data(self, state: AbroadDeepresearchState) -> dict:
        ...
        aggregated = AbroadAggregatedData(
            ...
            topGoodsRaw=state.category_top_goods_raw,
        )
    ...
    def _aggregate_property_data(self, state: AbroadDeepresearchState) -> dict:
        ...
        aggregated = AbroadAggregatedData(
            ...
            topGoodsRaw=state.property_top_goods_raw,
        )
        return {"aggregated_data": aggregated}
    ...
    def _generate_category_report_node(self, state: AbroadDeepresearchState) -> dict:
        ...
        # 修改商品卡数据为原数据
        replaced_text = self._replace_report_product_cards(report_text=report_text, origin_product_resp=aggregated_data.top_goods_raw)
    ...
    def _replace_report_product_cards(self, report_text: str, origin_product_resp: PageResult[AbroadGoodsEntity]) -> str:
    ...
    def _assemble_output_node(self, state: AbroadDeepresearchState) -> dict:
        ...
        if aggregated_data and param and pusher:
            exporter = AbroadExcelExporter(aggregated_data=aggregated_data, param=param, is_thinking=state.is_thinking)
