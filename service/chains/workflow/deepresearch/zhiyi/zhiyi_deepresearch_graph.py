...
from app.service.chains.workflow.deepresearch.zhiyi.schema import ZhiyiCategoryFormatItem, ZhiyiParsedCategory
...
class ZhiyiDeepresearchGraph(BaseWorkflowGraph):
    ...
    def _build_graph(self) -> CompiledStateGraph:
        ...
        graph.add_node(NodeKey.EXTRACT_PRICE_BAND, self._extract_price_band)
        ...
        graph.add_edge(NodeKey.CATEGORY_SHOP_AGGREGATE, NodeKey.CATEGORY_SHOP_REPORT)
        ...
        graph.add_edge(NodeKey.PROPERTY_SHOP_AGGREGATE, NodeKey.NORMAL_SHOP_REPORT)
        graph.add_edge(NodeKey.NORMAL_SHOP_REPORT, NodeKey.ASSEMBLE_OUTPUT)

        # 店铺 -> 价格带分析
        graph.add_edge(NodeKey.PRICE_SHOP_START, NodeKey.PRICE_SHOP_OVERVIEW)
        graph.add_edge(NodeKey.PRICE_SHOP_START, NodeKey.PRICE_SHOP_PRICE)
        graph.add_edge(NodeKey.PRICE_SHOP_START, NodeKey.PRICE_SHOP_TOP_GOODS)
        graph.add_edge(
        ...
        graph.add_edge(NodeKey.PRICE_SHOP_AGGREGATE, NodeKey.NORMAL_SHOP_REPORT)
        ...
        graph.add_edge(NodeKey.HYDC_CATEGORY_AGGREGATE, NodeKey.HYDC_CATEGORY_REPORT)
        ...
        graph.add_edge(NodeKey.HYDC_PROPERTY_AGGREGATE, NodeKey.HYDC_NORMAL_REPORT)
        ...
        graph.add_edge(NodeKey.HYDC_COLOR_AGGREGATE, NodeKey.HYDC_NORMAL_REPORT)
        ...
        graph.add_edge(NodeKey.HYDC_BRAND_AGGREGATE, NodeKey.HYDC_NORMAL_REPORT)

        # 大盘 -> 价格带分析 (table_type=3)
        graph.add_edge(NodeKey.HYDC_PRICE_START, NodeKey.HYDC_PRICE_OVERVIEW)
        graph.add_edge(NodeKey.HYDC_PRICE_START, NodeKey.HYDC_PRICE_DATA)
        graph.add_edge(NodeKey.HYDC_PRICE_START, NodeKey.HYDC_PRICE_TOP_GOODS)
        graph.add_edge(
        ...
        graph.add_edge(NodeKey.HYDC_PRICE_AGGREGATE, NodeKey.HYDC_NORMAL_REPORT)
    ...
    def _category_search_node(self, state: ZhiyiDeepresearchState):
        ...
        # 深度思考模式：推送数据收集阶段
        if pusher:
            category_name = parsed_category_list[0].category_name if parsed_category_list else "品类"
            pusher.push_phase(
                "数据收集",
                variables={"shop_name": category_name},
            )
    ...
    def _api_param_parse_node(self, state: ZhiyiDeepresearchState):
        ...
        recent_month_end = t_minus_1  # 近一个月结束日期：当前日期的前一天
        recent_month_start = t_minus_1 - timedelta(days=30)  # 近一个月开始日期：前一天的30天前
        ...
        invoke_params = {
            ...
            "recent_month_end_date": recent_month_end.strftime("%Y-%m-%d"),  # 近一个月结束日期
            "recent_month_start_date": recent_month_start.strftime("%Y-%m-%d"),  # 近一个月开始日期
            # 业务参数
        ...
        # 执行llm
        parse_param = struct_llm.with_config(run_name="解析api参数").invoke(messages)
        return {
    ...
    def _shop_search_node(self, state: ZhiyiDeepresearchState):
        ...
        # llm选择唯一店铺
        llm = llm_factory.get_llm(LlmProvider.OPENROUTER.name, LlmModelName.OPENROUTER_GEMINI_2_5_FLASH.value)
    ...
    def _query_shop_analyze_start(self, state: ZhiyiDeepresearchState):
        ...
        if pusher:
            pusher.push_phase(
                "洞察生成中",
                variables={"shop_name": shop_name or "", "insight_type": "店铺分析"},
            )
        return
    ...
    def _query_top_property(self, state: ZhiyiDeepresearchState):
        ...
        # 2. 使用LLM提取用户问题中的属性名
        property_list = property_list_resp.get("result", [])
        llm = llm_factory.get_llm(LlmProvider.OPENROUTER.name, LlmModelName.OPENROUTER_GEMINI_3_FLASH_PREVIEW.value)
        struct_llm = llm.with_structured_output(schema=PropertyExtractResult).with_retry(stop_after_attempt=2)

        invoke_params = {
            "user_query": req.user_query,
            "property_list": json.dumps(property_list, ensure_ascii=False),
        }
        messages: list[BaseMessage] = coze_loop_client_provider.get_langchain_messages(
        ...
        if not property_name and property_list:
            property_name = property_list[0].get("propertyName", "颜色")
    ...
    def _query_shop_analyze_top_goods(self, state: ZhiyiDeepresearchState):
        ...
        hot_item_resp = api_client.get_shop_hot_items(
            user_id=req.user_id,
            team_id=req.team_id,
    ...
    def _generate_shop_category_analyze_report(self, state: ZhiyiDeepresearchState):
        ...
        if pusher:
            pusher.push_phase(
                "洞察生成完成",
                variables={"shop_name": shop_name or ""},
            )
    ...
    def _analyze_hydc_dimension(self, state: ZhiyiDeepresearchState):
        ...
        result: HydcDimensionAnalyzeResult = struct_llm.with_config(run_name="大盘维度分析").invoke(messages)
    ...
    def _query_hydc_analyze_start(self, state: ZhiyiDeepresearchState):
        ...
        pusher = state.message_pusher
        param = state.api_parse_param
        category_name = param.category_id_name or param.root_category_id_name or "行业"

        if pusher:
            pusher.push_phase(
                "洞察生成中",
                variables={"shop_name": category_name, "insight_type": "大盘分析"},
            )
        return
    ...
    def _query_hydc_category_overview(self, state: ZhiyiDeepresearchState):
        ...
        # 销量趋势 (distribution=1)
        volume_request = ZhiyiHydcTrendRequest(
        ...
        volume_resp = api_client.get_hydc_sale_trend(
            user_id=req.user_id,
            team_id=req.team_id,
            params=volume_request,
        )

        # 销售额趋势 (distribution=3)
        amount_request = ZhiyiHydcTrendRequest(
            rootCategoryId=self._safe_int(param.root_category_id),
            categoryId=self._safe_int(param.category_id) if param.category_id else None,
            distribution=3,
            granularity=granularity,
        ...
        amount_resp = api_client.get_hydc_sale_trend(
            user_id=req.user_id,
            team_id=req.team_id,
            params=amount_request,
        )
        ...
        return {
            "hydc_category_overview_volume": self._clean_overview_data(volume_resp),
            "hydc_category_overview_amount": self._clean_overview_data(amount_resp),
        }
    ...
    def _query_hydc_category_price(self, state: ZhiyiDeepresearchState):
        ...
        price_request = ZhiyiHydcPriceRangeRequest(
            rootCategoryId=self._safe_int(param.root_category_id),
            categoryId=self._safe_int(param.category_id) if param.category_id else None,
            startDate=param.start_date,
            endDate=param.end_date,
        ...
        price_resp = api_client.get_hydc_price_range_trend(
            user_id=req.user_id,
            team_id=req.team_id,
            params=price_request,
        )
        ...
        return {
            "hydc_category_price_data": self._clean_price_range_data(price_resp),
        }
    ...
    def _query_hydc_category_color(self, state: ZhiyiDeepresearchState):
        ...
        color_request = ZhiyiHydcPropertyRequest(
            rootCategoryId=self._safe_int(param.root_category_id),
            categoryId=param.category_id,
            startDate=param.start_date,
            endDate=param.end_date,
        ...
        color_resp = api_client.get_hydc_property_trend(
            user_id=req.user_id,
            team_id=req.team_id,
            params=color_request,
        )
        ...
        return {
            "hydc_category_color_data": self._clean_color_data(color_resp),
        }
    ...
    def _query_hydc_category_category(self, state: ZhiyiDeepresearchState):
        ...
        category_request = ZhiyiHydcCategoryRequest(
            startDate=param.start_date,
            endDate=param.end_date,
            rootCategoryId=self._safe_int(param.root_category_id) if param.root_category_id else None,
            categoryId=self._safe_int(param.category_id) if param.category_id else None,
            granularity=granularity,
        )
        category_resp = api_client.get_hydc_category_trend(
            user_id=req.user_id,
            team_id=req.team_id,
            params=category_request,
        )
        ...
        return {
            "hydc_category_category_data": self._clean_category_trend_data(category_resp),
        }
    ...
    def _query_hydc_category_top_goods(self, state: ZhiyiDeepresearchState):
        ...
        hot_item_request = ZhiyiHydcTopItemRequest(
            rootCategoryIdList=[root_category_id] if root_category_id else [],
            categoryIdList=[self._safe_int(category_id)] if category_id else None,
            startDate=param.start_date,
            endDate=param.end_date,
        )
        hot_item_resp = api_client.get_hydc_hot_items(
            user_id=req.user_id,
            team_id=req.team_id,
            params=hot_item_request,
        )
        ...
        return {
            "hydc_category_top_goods": self._clean_top10_items_data(hot_item_resp),
        }
    ...
    def _generate_hydc_category_report(self, state: ZhiyiDeepresearchState):
        ...
        invoke_params = {
            ...
            "category_trend_data": state.hydc_category_category_data.model_dump_json(ensure_ascii=False) if state.hydc_category_category_data else "{}",
            ...
            "top_goods_data": state.hydc_category_top_goods.model_dump_json(ensure_ascii=False) if state.hydc_category_top_goods else "{}",
        }
        messages = coze_loop_client_provider.get_langchain_messages(
    ...
    def _query_hydc_property_start(self, state: ZhiyiDeepresearchState):
        ...
        if pusher:
            pusher.push_phase(
                "洞察生成中",
                variables={"shop_name": category_name, "insight_type": "属性分析"},
            )
        return
    ...
    def _extract_hydc_property(self, state: ZhiyiDeepresearchState):
        ...
        property_top_request = ZhiyiPropertyTopRequest(
            rootCategoryId=self._safe_int(param.root_category_id),
            categoryId=self._safe_int(param.category_id) if param.category_id else None,
            rootCategoryIdList=self._safe_int(param.root_category_id),
        )
        property_list_resp = api_client.get_property_top_list(
            user_id=req.user_id,
            team_id=req.team_id,
        ...
        return {"hydc_property_name": property_name}
    ...
    def _query_hydc_property_overview(self, state: ZhiyiDeepresearchState):
        ...
        return {
            "hydc_property_overview_volume": self._clean_overview_data(volume_resp),
            "hydc_property_overview_amount": self._clean_overview_data(amount_resp),
        }
    ...
    def _query_hydc_property_data(self, state: ZhiyiDeepresearchState):
        ...
        property_request = ZhiyiHydcPropertyRequest(
            rootCategoryId=self._safe_int(param.root_category_id),
            categoryId=param.category_id,
            startDate=param.start_date,
            endDate=param.end_date,
        ...
        property_resp = api_client.get_hydc_property_trend(
            user_id=req.user_id,
            team_id=req.team_id,
            params=property_request,
        )
        ...
        return {
            "hydc_property_data": self._clean_color_data(property_resp),
        }
    ...
    def _query_hydc_property_top_goods(self, state: ZhiyiDeepresearchState):
        ...
        return {
            "hydc_property_top_goods": self._clean_top10_items_data(hot_item_resp),
        }
    ...
    def _query_hydc_color_start(self, state: ZhiyiDeepresearchState):
        ...
        if pusher:
            pusher.push_phase(
                "洞察生成中",
                variables={"shop_name": category_name, "insight_type": "颜色分析"},
            )
        return
    ...
    def _query_hydc_color_overview(self, state: ZhiyiDeepresearchState):
        ...
        return {
            "hydc_color_overview_volume": self._clean_overview_data(volume_resp),
            "hydc_color_overview_amount": self._clean_overview_data(amount_resp),
        }
    ...
    def _query_hydc_color_data(self, state: ZhiyiDeepresearchState):
        ...
        return {
            "hydc_color_data": self._clean_color_data(color_resp),
        }
    ...
    def _query_hydc_color_top_goods(self, state: ZhiyiDeepresearchState):
        ...
        return {
            "hydc_color_top_goods": self._clean_top10_items_data(hot_item_resp),
        }
    ...
    def _query_hydc_brand_start(self, state: ZhiyiDeepresearchState):
        ...
        if pusher:
            pusher.push_phase(
                "洞察生成中",
                variables={"shop_name": category_name, "insight_type": "品牌分析"},
            )
        return
    ...
    def _query_hydc_brand_overview(self, state: ZhiyiDeepresearchState):
        ...
        return {
            "hydc_brand_overview_volume": self._clean_overview_data(volume_resp),
            "hydc_brand_overview_amount": self._clean_overview_data(amount_resp),
        }
    ...
    def _query_hydc_brand_data(self, state: ZhiyiDeepresearchState):
        ...
        brand_request = ZhiyiHydcBrandRequest(
            rootCategoryId=self._safe_int(param.root_category_id),
            categoryId=self._safe_int(param.category_id) if param.category_id else None,
            startDate=param.start_date,
            endDate=param.end_date,
        ...
        brand_resp = api_client.get_hydc_brand_trend(
            user_id=req.user_id,
            team_id=req.team_id,
        ...
        return {
            "hydc_brand_data": self._clean_brand_data(brand_resp),
        }
    ...
    def _query_hydc_brand_top_goods(self, state: ZhiyiDeepresearchState):
        ...
        return {
            "hydc_brand_top_goods": self._clean_top10_items_data(hot_item_resp),
        }
    ...
    def _query_hydc_price_start(self, state: ZhiyiDeepresearchState):
        ...
        if pusher:
            pusher.push_phase(
                "洞察生成中",
                variables={"shop_name": category_name, "insight_type": "价格带分析"},
            )
        return
    ...
    def _query_hydc_price_overview(self, state: ZhiyiDeepresearchState):
        ...
        return {
            "hydc_price_overview_volume": self._clean_overview_data(volume_resp),
            "hydc_price_overview_amount": self._clean_overview_data(amount_resp),
        }
    ...
    def _query_hydc_price_data(self, state: ZhiyiDeepresearchState):
        ...
        return {
            "hydc_price_data": self._clean_price_range_data(price_resp),
        }
    ...
    def _query_hydc_price_top_goods(self, state: ZhiyiDeepresearchState):
        ...
        return {
            "hydc_price_top_goods": self._clean_top10_items_data(hot_item_resp),
        }
    ...
    def _generate_hydc_normal_report(self, state: ZhiyiDeepresearchState):
        ...
        if table_type == "3":  # 价格带分析
            ...
            dimension_name = state.hydc_property_name or "属性"
        ...
        invoke_params = {
            ...
            "dimension_data": dimension_data.model_dump_json(ensure_ascii=False) if dimension_data else "{}",
            "top_goods_data": top_goods.model_dump_json(ensure_ascii=False) if top_goods else "{}",
        }
        messages = coze_loop_client_provider.get_langchain_messages(
