...
class DouyiDeepresearchGraph(BaseWorkflowGraph):
    ...
    def _query_category_analyze_data(self, state: DouyiDeepresearchState):
        ...
        volume_request = DouyiTrendAnalysisRequest(
            ...
            relateType=param.frontTitleTabValue,
            target="saleVolume",
        )
    ...
    def _get_default_property(self, selector_resp: DouyiPropertySelectorRawResponse | None) -> DouyiExtractedProperty:
        ...
        property_name = first_prop.property_name or "颜色"

        # 构建属性值列表
        properties = []
        if first_prop.property_value_list:
            for val in first_prop.property_value_list:
    ...
    def _extract_price_range_with_llm(self, user_query: str) -> DouyiExtractedPriceRange:
        ...
        # 获取LLM
        llm = llm_factory.get_llm(LlmProvider.OPENROUTER.name, LlmModelName.OPENROUTER_GEMINI_3_FLASH_PREVIEW.value)
        struct_llm = llm.with_structured_output(schema=DouyiExtractedPriceRange).with_retry(stop_after_attempt=2)
    ...
    def _query_top_goods_data(self, state: DouyiDeepresearchState):
        ...
        # 查询Top商品
        top_request = DouyiTopItemsRequest(
            relateType="windowGoods",
            startDate=param.start_date,
            endDate=param.end_date,
            rootCategoryId=param.root_category_id,
            categoryIdList=category_id_list,
            target="cardSaleVolume",
            queryType="itemAnalysis",
            limit=10,
            pageNo=1,
            pageSize=20,
        )
        top_resp = api_client.get_douyi_top_items(
            user_id=req.user_id,
            team_id=req.team_id,
            params=top_request,
        )
        ...
        # 清洗数据
        top_clean = self._clean_top_goods_data(top_resp)

        # 根据当前分支返回不同的state字段
        table_type = param.table_type
        if table_type == DouyiTableType.CATEGORY_ANALYSIS.code:
            return {"category_top_goods": top_clean, "category_top_resp": top_resp}
        elif table_type == DouyiTableType.PROPERTY_ANALYSIS.code:
            return {"property_top_goods": top_clean, "property_top_resp": top_resp}
        else:
            return {"price_top_goods": top_clean, "price_top_resp": top_resp}
    ...
    def _clean_top_goods_data(self, raw_response: DouyiTopItemsRawResponse) -> DouyiTopGoodsCleanResponse:
        """清洗Top商品数据 - 对应n8n的Code in JavaScript2，参照JS清洗代码逻辑"""
        if not raw_response.result or not raw_response.result.result_list:
        ...
        top_items = []
        for idx, item in enumerate(raw_response.result.result_list[:10]):
            # 获取抖音商品嵌套数据
        ...
        result = DouyiTopGoodsSlimResult(
            start=raw_response.result.start or 0,
            pageSize=raw_response.result.page_size or 10,
            resultCount=len(raw_response.result.result_list),
    ...
    def _replace_report_product_cards(self, report_text: str, origin_product_resp: DouyiTopItemsRawResponse) -> str:
        new_product_card_content = f"<custom-productcards>{origin_product_resp.model_dump_json(by_alias=True, ensure_ascii=False)}</custom-productcards>"
    ...
    def _assemble_output_node(self, state: DouyiDeepresearchState):
        ...
        # 构建 Excel 导出数据
        exporter = DouyiExcelExporter(aggregated_data, param)
