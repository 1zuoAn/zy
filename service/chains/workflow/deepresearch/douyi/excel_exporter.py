...
class DouyiExcelExporter:
    ...
    def __init__(self, aggregated_data: DouyiAggregatedData, param: DouyiMainParseParam):
    ...
    def build_excel_data_list(self) -> List[ExcelData]:
        ...
        if self.data.top10_products and self.data.top10_products.result:
    ...
    def _generate_filename(self, data_type: str) -> str:
        ...
        month_str = self._extract_month_str()
        return f"{category_name}{month_str}{data_type}.xlsx"

    def _extract_month_str(self) -> str:
        """从参数中提取月份字符串，如 '11月'"""
        if self.param.end_date:
            try:
                end_date = datetime.strptime(self.param.end_date, "%Y-%m-%d")
                return f"{end_date.month}月"
            except ValueError:
                pass
        return ""
    ...
    def _build_sales_trend_sheet(self) -> SheetData:
        ...
        列结构: 日期 | 销量 | 直播销量 | 作品销量 | 商品卡销量 | 销售额(元)
        """
        columns = [
            ...
            ColumnInfo(col_name="直播销量", col_id=3, col_type=EXPORT_COLUMN_TYPE_STRING),
            ColumnInfo(col_name="作品销量", col_id=4, col_type=EXPORT_COLUMN_TYPE_STRING),
            ColumnInfo(col_name="商品卡销量", col_id=5, col_type=EXPORT_COLUMN_TYPE_STRING),
        ...
        for row_idx, item in enumerate(volume_data, start=1):
            ...
            cells = [
                ...
                CellData(col_id=3, cell_value=str(item.daily_live_sum)),
                CellData(col_id=4, cell_value=str(item.daily_video_sum)),
                CellData(col_id=5, cell_value=str(item.daily_card_sum)),
                CellData(col_id=6, cell_value=f"{amount_yuan:.2f}"),
            ]
    ...
    def _build_top_goods_sheet(self) -> SheetData:
        ...
        columns = [
            ...
            ColumnInfo(col_name="店铺", col_id=8, col_type=EXPORT_COLUMN_TYPE_STRING),
            ColumnInfo(col_name="品牌", col_id=9, col_type=EXPORT_COLUMN_TYPE_STRING),
        ]
        ...
        for row_idx, item in enumerate(top_goods, start=1):
            ...
            cells = [
                ...
                CellData(col_id=4, cell_value=item.category_detail or ""),
                CellData(col_id=5, cell_value=str(item.sales_volume)),
                CellData(col_id=6, cell_value=f"{sales_amount_yuan:.2f}"),
                CellData(col_id=7, cell_value=f"{price_yuan:.2f}"),
                CellData(col_id=8, cell_value=item.shop_name or ""),
                CellData(col_id=9, cell_value=item.brand or ""),
            ]
            rows.append(RowData(row_id=row_idx, row_data=cells))
