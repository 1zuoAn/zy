...
from app.core.tools.business_tools import ALL_TOOLS
# 意图到工作流类型的映射
...
class MainOrchestratorGraph(BaseWorkflowGraph):
    ...
    def _build_graph(self) -> CompiledStateGraph:
        ...
        llm = llm_factory.LLMFactory.create_llm(
            provider=LlmProvider.HUANXIN.name,
            model=LlmModelName.HUANXIN_GPT_4O.value,
            max_tokens=4096
        ...
        # [核心] Agent 思考节点
        graph.add_node("agent", self._agent_think_node)
        
        # [核心] 工具执行节点 (自动执行 @tool 函数)
        ...
        # 准备好历史后，进入 Agent 思考
        graph.add_edge("query_history", "agent")

        # [核心] Agent 循环逻辑
        # tools_condition 判断：
        # - 如果 LLM 想调工具 -> 走 "tools"
        # - 如果 LLM 说完了 -> 走 "summary" (结束)
        graph.add_conditional_edges(
            "agent",
            tools_condition,
            {"tools": "tools", "__end__": "summary"}
        )

        # 工具执行完，结果必须回传给 Agent 继续思考 (Loop)
    ...
    # ==================== 节点实现 ====================
    def _agent_think_node(self, state: MainOrchestratorState) -> Dict[str, Any]:
        """
        Agent 思考节点 (LLM)
        """
        messages = state.get("messages", [])
        
        # 如果 messages 为空 (第一次进入)，需要构建初始 Prompt
        if not messages:
            req = state["request"]
            image_content = state.get("image_content", "")
            history = state.get("conversation_history", "")
            
            system_prompt = f"""你是一个服装行业智能助手。
            你的任务是根据用户需求，自主规划并调用合适的工具来解决问题。
            
            【上下文信息】
            - 用户偏好平台: {req.preferred_entity or '未指定'}
            - 行业: {req.industry}
            - 图片特征: {image_content}
            - 历史对话: {history}
            
            【工作原则】
            1. 能够识别并拆解复杂任务（如"先查趋势，再根据趋势选品"）。
            2. 如果用户没有指定平台，请根据工具描述合理选择默认工具。
            3. 每次工具调用结束后，根据工具返回的结果，决定是继续下一步还是总结回答。
            """
    ...
    def _summary_node(self, state: MainOrchestratorState) -> Dict[str, Any]:
        ...
        # 【TODO】这里未来可以解析 artifacts，如果有生成的图片/报表，在这里提取出来传给 workflow_response
        
        logger.debug(f"[Agent总结] {summary_text}")
