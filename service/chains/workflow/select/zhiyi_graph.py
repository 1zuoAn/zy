...
class ZhiyiGraph(BaseWorkflowGraph):
    ...
    def _llm_param_parse_node(self, state: ZhiyiWorkflowState) -> dict[str, Any]:
        ...
        return {
            ...
            "properties_parse_result": properties_result,
        }
    ...
    def _llm_param_merge_node(self, state: ZhiyiWorkflowState) -> dict[str, Any]:
        ...
        prompt_param = {
            "user_query": format_query,
            "current_query_title": param_result.query_title or "",
            ...
            "properties_parse_result": dump_prompt_value(state.get("properties_parse_result")),
        }
    ...
    def _parallel_parse_node(self, state: ZhiyiWorkflowState) -> dict[str, Any]:
        ...
        def has_shop_hint(text: str) -> bool:
            if not text:
                return False
            shop_hints = ("官方旗舰店", "旗舰店", "官方店", "专卖店", "店铺")
            return any(hint in text for hint in shop_hints)

        def extract_shop_name_from_query(text: str) -> str | None:
            if not text:
                return None
            pattern = re.compile(
                r"([\\w\\u4e00-\\u9fff/·\\-]{2,30}(?:官方旗舰店|旗舰店|官方店|专卖店|店铺))"
            )
        ...
        def query_shop_id() -> str | None:
            ...
            try:
                ...
                if not shop_ref:
                    if not has_shop_hint(format_query):
                        return None
                    shop_name = extract_shop_name_from_query(format_query) or format_query
        ...
        def parse_properties() -> list[dict[str, str]]:
            ...
            try:
                ...
                def _recall_property_candidates_from_vector() -> list[str]:
                    kb_client = get_volcengine_kb_api()
                    response = kb_client.simple_chat(
                        query=param_result.properties,
                        service_resource_id=VolcKnowledgeServiceId.ZHIYI_PROPERTIES_VECTOR.value,
                    )
                    if not response.data or not response.data.result_list:
                        return []
                    content_list: list[str] = []
                ...
                allowed: dict[str, set[str] | None] = {}
                try:
                    # 优先从商品库接口按类目拉取可用属性候选，避免出现“商品库该类目下不存在的筛选项”。
                    if root_category_id and category_id_list:
                    ...
                    logger.warning(f"[知衣工作流属性解析] 类目属性候选接口不可用，降级使用向量召回：{e}")

                if not content_list:
                    # 降级：接口不可用或候选为空时，回退到属性向量召回，保证流程可用。
        ...
        def parse_properties_legacy() -> list[dict[str, str]]:
        ...
        property_task = parse_properties_legacy if shop_id else parse_properties
        property_future = thread_pool.submit_with_context(property_task)
