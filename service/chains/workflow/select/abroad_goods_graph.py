...
class AbroadGoodsGraph(BaseWorkflowGraph):
    ...
    def _merge_param_result(
        ...
        *,
        ...
        platform_name_hint = (
            selection_dict.get("platform_name_override")
            or selection_dict.get("site_name")
        )
        if (
            platform_name_hint
            and param_result.brand
            and not selection_dict.get("brand_name")
        ):
            brand_norm = normalize_entity_name(param_result.brand)
            platform_norm = normalize_entity_name(platform_name_hint)
            if brand_norm and platform_norm and (
                brand_norm in platform_norm or platform_norm in brand_norm
            ):
                logger.debug(f"[海外探款] 清除被误解析为品牌的平台名: {param_result.brand}")
                param_result.brand = None
    ...
    def _llm_param_merge_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        prompt_param = {
            ...
            "min_sprice": _stringify_number(param_result.min_sprice),
            "max_sprice": _stringify_number(param_result.max_sprice),
    ...
    def _merge_search_result_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        if resolved_request_body is not None and not isinstance(resolved_request_body, str):
            resolved_request_body = json.dumps(resolved_request_body, ensure_ascii=False)
    ...
    def _has_result_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        if fallback_attempted:
            if not state.get("fallback_progress_pushed"):
        ...
        if not request_path:
            logger.warning("[海外探款商品] request_path缺失，跳过输出参数推送")
        ...
        if data_payload and request_path:
        ...
        elif request_path and not request_body:
            logger.warning("[海外探款商品] request_body缺失，跳过输出参数推送")
    ...
    def _no_result_node(self, state: AbroadGoodsWorkflowState) -> dict[str, Any]:
        ...
        fallback_api_request = state.get("fallback_api_request")
        platform_name = state.get("platform_name", "海外探款")
        ...
        with pg_session(DBAlias.DB_ABROAD_AI.value) as session:
            session.execute(
                     ...
                     """),
                {
                    "session_id": req.session_id,
                    "message_id": req.message_id,
                    "api_request": fallback_api_request.model_dump_json(by_alias=True, exclude_none=True),
