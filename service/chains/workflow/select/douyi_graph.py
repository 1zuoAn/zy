...
from app.service.rpc.zhiyi.client import get_zhiyi_api_client as get_zhiyi_common_api_client
from app.service.rpc.zhiyi.schemas import DouyiPropertySelectorRequest
...
class DouyiGraph(BaseWorkflowGraph):
    ...
    def _build_graph(self) -> CompiledStateGraph:
        ...
        # 调整顺序：先做属性解析(parallel_parse)，再做参数合并(llm_merge)
        # 这样合并节点可以拿到属性清洗结果进行关键词修正
        graph.add_edge("llm_parse", "parallel_parse")
        graph.add_edge("parallel_parse", "llm_merge")
        graph.add_edge("llm_merge", "api_strict")
    ...
    def _check_strict_result(self, state: DouyiWorkflowState) -> str:
        ...
        if not api_success or (result_count is not None and result_count < STRICT_RESULT_MIN_COUNT):
            return "no_result"
        return "has_result"
    ...
    def _llm_param_merge_node(self, state: DouyiWorkflowState) -> dict[str, Any]:
        """LLM 合并节点：修正冲突/过窄条件 + 关键词修正

        注意：此节点现在在 parallel_parse 之后执行，可以拿到属性清洗结果
        """
        req = state["request"]
        param_result: DouyiParseParam = state["param_result"]
        selection_dict = state.get("selection_dict") or {}

        # 获取属性清洗结果（来自 parallel_parse）
        property_list_by_category = state.get("property_list_by_category") or {}
        unmatched_keywords = state.get("unmatched_keywords") or []
        ...
        category_names = ""
        raw_category_list = selection_dict.get("category_data", [])
        if raw_category_list:
            names = []
            for item in raw_category_list:
                if isinstance(item, str) and "#" in item:
                    name_part = item.split("#")[0].strip()
                    if name_part and name_part not in names:
                        names.append(name_part)
            category_names = ",".join(names[:5])

        # 提取已匹配的属性名称
        matched_properties = ""
        if property_list_by_category:
            prop_names = set()
            for props in property_list_by_category.values():
                for prop_group in props:
                    for prop in prop_group:
                        if ":" in prop:
                            prop_names.add(prop.split(":")[0])
            matched_properties = ",".join(list(prop_names)[:5])

        # 将未匹配关键词传给 LLM 进行校验/修正
        unmatched_keywords_text = ",".join(unmatched_keywords[:5]) if unmatched_keywords else ""
        ...
        prompt_param = {
            ...
            "matched_properties": matched_properties,  # 新增：已匹配的属性名称
            "unmatched_keywords": unmatched_keywords_text,  # 新增：未匹配的关键词（待修正）
        ...
        try:
            ...
            # 合并失败时，使用原始的 unmatched_keywords 作为关键词
            merge_keyword = " ".join(unmatched_keywords[:3]) if unmatched_keywords else None
            return {"merge_keyword": merge_keyword}

        # 提取修正后的 keyword
        merge_keyword = None
        if merge_result.keyword:
            merge_keyword = merge_result.keyword.strip()
        # 如果 LLM 没有返回 keyword，但有 unmatched_keywords，使用它们
        if not merge_keyword and unmatched_keywords:
            merge_keyword = " ".join(unmatched_keywords[:3])

        if merge_keyword:
            logger.debug(f"[抖衣] 合并节点修正关键词: {merge_keyword}")
        ...
        return {"param_result": updated, "merge_keyword": merge_keyword}
    ...
    def _parallel_parse_node(self, state: DouyiWorkflowState) -> dict[str, Any]:
        ...
        futures = {}
        category_ids_for_properties: list[int] = []

        # 属性解析
        category_ids_for_properties = self._normalize_category_id_list(param_result.category_id_list)
        ...
        if root_category_id is not None or category_ids_for_properties:
            futures["properties"] = thread_pool.submit_with_context(
                self._parse_properties, req, format_query, param_result
            )
        ...
        property_list_by_category: dict[int, list[list[str]]] = {}
        user_tags: list[str] = []
        sort_result: DouyiSortTypeParseResult | None = None
        user_filters: list[Any] = []
        unmatched_keywords: list[str] = []  # 新增：未匹配的关键词

        for key, future in futures.items():
            try:
                result = future.result(timeout=20)
                if key == "properties":
                    # _parse_properties 现在返回 (property_list, unmatched_keywords) 元组
                    if isinstance(result, tuple) and len(result) == 2:
                        property_list = result[0] or []
                        unmatched_keywords = result[1] or []
                    else:
                        # 兼容旧格式
        ...
        if property_list:
            if category_ids_for_properties:
                for category_id in category_ids_for_properties:
                    property_list_by_category[category_id] = property_list
            else:
                root_category_id = param_result.root_category_id
                if root_category_id is None:
                    industry = getattr(req, "industry", "") or ""
                ...
                if root_category_id is not None:
                    property_list_by_category[root_category_id] = property_list

        if property_list_by_category:
            property_list = self._resolve_property_list(
                property_list_by_category, category_ids_for_properties, property_list
            )
        ...
        self._track_llm_parameters(
            ...
            param_result,
            property_list,
            property_list_by_category or None,
            sort_result,
            user_tags,
            user_filters,
        )
        ...
        logger.debug(
            f"[抖衣] Step 5: parallel_parse - 属性={property_list}, 用户画像标签={user_tags}, 排序={sort_result}, 未匹配关键词={unmatched_keywords}"
        )
        return {
            # "property_list": property_list,
            "property_list_by_category": property_list_by_category or None,
            "sort_param_result": sort_result,
            "user_filters": user_filters,
            "user_tags": user_tags,
            "unmatched_keywords": unmatched_keywords,  # 新增：传递给合并节点
        }
    ...
    def _parse_properties(
        ...
        Returns:
            tuple: (property_list, unmatched_keywords)
                - property_list: 二维属性列表
                - unmatched_keywords: 未匹配到属性的商品特征词
        """
        try:
            cleaned_text = (origin_text or "").replace("\n", " ").strip()
            if not cleaned_text:
                return [], []
            ...
            try:
                ...
                if root_category_id is None:
                    ...
                    if "#" in industry:
                        ...
                        if tail.isdigit():
                            root_category_id = int(tail)
                category_ids = self._normalize_category_id_list(param_result.category_id_list)
                category_id = category_ids[0] if category_ids else None

                if root_category_id:
                    api_client = get_zhiyi_common_api_client()
                    selector_request = DouyiPropertySelectorRequest(
                        rootCategoryId=root_category_id,
                        categoryId=category_id if not category_ids else None,
                        categoryIdList=category_ids or None,
                    )
                    selector_resp = api_client.get_douyi_property_selector(
                        user_id=req.user_id,
                        team_id=req.team_id,
                        params=selector_request,
                    )
                    if selector_resp and selector_resp.result:
                        options = [
                            item.model_dump(by_alias=True, exclude_none=True)
                            for item in selector_resp.result
                        ]
                        content_list, allowed = self._normalize_property_options_to_candidates(options)
            except Exception as e:
                logger.warning(f"[抖衣属性标签解析] 类目属性候选接口不可用: {e}")

            if not content_list:
                return [], []

            # 2. llm标签清洗（同时提取未匹配的关键词）
            structured_llm = (
                llm_factory.get_llm(
                    LlmProvider.DASHSCOPE.name, LlmModelName.DASHSCOPE_QWEN3_MAX.value
                )
                ...
                .with_structured_output(schema=DouyiPropertyLlmCleanResult)
            ...
            # 提取未匹配的关键词
            unmatched_keywords = result.unmatched_keywords or []
            if unmatched_keywords:
                logger.debug(f"[抖衣属性解析] 提取未匹配关键词: {unmatched_keywords}")
            ...
            return grouped_property_list, unmatched_keywords
        except Exception as e:
    ...
    @staticmethod
    def _normalize_category_id_list(raw_list: list[Any] | None) -> list[int]:
        """将类目ID列表规整为整数列表"""
        if not raw_list:
            return []
        ids: list[int] = []
    ...
    def _resolve_property_list(
        self,
        property_list_by_category: dict[int, list[list[str]]],
        category_id_list: list[Any] | None,
        fallback: list[list[str]] | None = None,
    ) -> list[list[str]]:
        """从类目属性映射中合并共同属性（同名属性取值并集）"""
        if property_list_by_category:
            merged = self._merge_common_property_list(property_list_by_category, category_id_list)
            return merged
        return fallback or []

    def _merge_common_property_list(
        self,
        property_list_by_category: dict[int, list[list[str]]],
        category_id_list: list[Any] | None,
    ) -> list[list[str]]:
        """合并各类目共同属性"""
        category_ids = self._normalize_category_id_list(category_id_list)
        if not category_ids:
            category_ids = list(property_list_by_category.keys())
        if not category_ids:
            return []

        key_order: list[str] = []
        per_category: list[dict[str, list[str]]] = []

        for category_id in category_ids:
            groups = property_list_by_category.get(category_id) or []
            ...
            for group in groups:
                ...
                for tag in group:
                    ...
                    if not key or not value:
                        continue
                    values = key_map.setdefault(key, [])
                    if value not in values:
                        values.append(value)
                    if key not in key_order:
                        key_order.append(key)
            per_category.append(key_map)

        if not per_category:
            return []

        common_keys = set(per_category[0].keys())
        for key_map in per_category[1:]:
            common_keys.intersection_update(key_map.keys())
        if not common_keys:
            return []
        ...
        for key in key_order:
            ...
            for key_map in per_category:
                for value in key_map.get(key, []):
                    if value in seen_values:
                        continue
                    seen_values.add(value)
                    merged_values.append(value)
            if merged_values:
                merged.append([f"{key}:{value}" for value in merged_values])
        return merged
    ...
    def _track_llm_parameters(
        ...
        param_result: DouyiParseParam,
        property_list: list[list[str]],
        property_list_by_category: dict[int, list[list[str]]] | None,
        ...
        property_list = self._resolve_property_list(
            property_list_by_category or {}, param_result.category_id_list, property_list
        )
    ...
    def _api_strict_node(self, state: DouyiWorkflowState) -> dict[str, Any]:
        ...
        try:
            property_list = self._resolve_property_list(
                property_list_by_category, param_result.category_id_list, property_list
            )
            if shop_id:
                ...
                # 使用合并节点提取的关键词（如果存在）
                merge_keyword = state.get("merge_keyword") or ""
                api_request.keyword = merge_keyword
    ...
    def _api_fallback_node(self, state: DouyiWorkflowState) -> dict[str, Any]:
        ...
        try:
            if shop_id:
                ...
                # 优先使用合并节点提取的关键词，否则从属性提取
                merge_keyword = state.get("merge_keyword") or ""
                if not merge_keyword:
                    property_list = self._resolve_property_list(
                        property_list_by_category, param_result.category_id_list, property_list
                    )
                    merge_keyword = self._extract_keyword_from_properties(property_list) or ""
                api_request.keyword = merge_keyword
    ...
    def _merge_result_node(self, state: DouyiWorkflowState) -> dict[str, Any]:
        ...
        if api_success and result_count >= STRICT_RESULT_MIN_COUNT:
            final_resp = api_resp
    ...
    def _no_result_node(self, state: DouyiWorkflowState) -> dict[str, Any]:
        """无结果分支"""
        req = state["request"]
        fallback_request = state["fallback_api_request"]
        ...
        with pg_session(DBAlias.DB_ABROAD_AI.value) as session:
            session.execute(
                     ...
                     """),
                {
                    "session_id": req.session_id,
                    "message_id": req.message_id,
                    "api_request": fallback_request.model_dump_json(by_alias=True, exclude_none=True),
