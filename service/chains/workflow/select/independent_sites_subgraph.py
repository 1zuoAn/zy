...
class IndependentSitesSubGraph:
    ...
    def _should_use_put_on_sale(
        self, state: IndependentSitesSubGraphState, param_result: Any
    ) -> bool:
        query = str(state.get("user_query") or "")
        if query:
            keywords = ("上新", "上架", "新品", "上市", "新上", "新款", "上架时间")
            if any(keyword in query for keyword in keywords):
                return True
        new_type = getattr(param_result, "new_type", None)
        if isinstance(new_type, str) and "新品" in new_type:
            return True
        return False
    ...
    def _build_api_params(
        ...
        if param_result:
            ...
            # 上架日期
            if self._should_use_put_on_sale(state, param_result):
